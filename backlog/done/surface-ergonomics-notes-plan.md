...
---
- 2026-08-19T03:40:45Z advanced to=doing claimer=fairchild@blue branch=demo
- 2026-08-19T07:51:01Z progress | index/generate_notes.py (uv run --with markdown-it-py) extracts a named allowlist of headings (Ergonomics notes + 7 named neighbours) from all 9 stack READMEs, renders per-stack + grouped-overview pages matching golden.html's tokens; wired into scripts/run.sh (index/notes/, gitignored) and scripts/hosted.sh build_index (dist/notes/, plus copying every sibling *.html and dereferencing every *.gif into dist/ — covers monolith.html too, coordinated with demo-78). index/index.html links notes from every backend/frontend header, the jinja outside card (restructured off a nested-anchor), and the More section. Verified: READMEs unchanged after build, missing-Ergonomics-notes fails loudly, editing a section changes only that page, screenshots light+dark, and live click-through on both :3000 (local) and :4000 (hosted preview, wrangler dev hot-reloads dist/).
- 2026-08-19T07:51:33Z advanced to=done
