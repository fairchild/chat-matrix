...
- 2026-08-20T04:40:02Z progress | Decided and shipped (f8f8576). Both routes bound to a var defaulting closed, so wrangler deploy reads the safe shape from the committed config and bun run dev carries the unlock. /threads -> [] + detail, POST /model -> 403, GET /models -> locked+why, hub draws a fixed chip. Third exposure (POST /model) folded in rather than deferred. Verified: locked vs unlocked wrangler dev, all four backends 21/21 via run.sh, golden 40/40, probes 29 pass. Token exposure is structurally zero either way (no provider secret deployed; unavailable() gates on bindings) — the lock is what keeps that true if a key ever lands. Cost recorded: conformance skips the thread-list assertion against a locked backend, only when the backend states a reason.
- 2026-08-20T04:40:02Z advanced to=done
