/**
 * AG-UI, from pi's session events. Event names follow the AG-UI core spec; the
 * request shape is `RunAgentInput` (`threadId`, `runId`, `messages`).
 */
        ...
        case "message_update": {
          ...
          switch (e.type) {
            ...
            case "toolcall_start": {
              ...
              return [
                stamp({ type: "TOOL_CALL_START", toolCallId: call.id, toolCallName: call.name, parentMessageId }),
              ];
            }
            case "toolcall_delta": {
              const call = e.partial.content[e.contentIndex] as ToolCall;
              return [stamp({ type: "TOOL_CALL_ARGS", toolCallId: call.id, delta: e.delta })];
            }
            case "toolcall_end":
              return [stamp({ type: "TOOL_CALL_END", toolCallId: e.toolCall.id })];
            default:
              return [];
          }
        }
