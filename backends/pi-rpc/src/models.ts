 ...
 *
 * Same catalogue as the in-process `pi` backend, and the same credential sources:
 * a login written by `pi auth login` counts, so a provider can be available here
 * with no environment variable involved, and OpenAI arrives two ways — an API key
 * on `openai` or a Codex login on `openai-codex`. The check runs in this process
 * even though the model is used in a child, because a child's `--model` is fixed
 * at spawn and the answer needs to come back before one is spawned.
 */
...
/** The child registers the scripted provider from `extension.ts`; this parent only names it. */
export const SCRIPTED_SPEC = `${SCRIPTED}/${SCRIPTED}`;
...
/** Auth only — the child does the real resolution, so this runtime never streams. */
export const authRuntime = () => ModelRuntime.create();
...
/** A catalogue id as the child's `--model` argument; anything else passes through unchanged. */
export function spec(runtime: ModelRuntime, id: string): string {
  if (id === SCRIPTED) return SCRIPTED_SPEC;
  const candidate = BY_ID.get(id);
