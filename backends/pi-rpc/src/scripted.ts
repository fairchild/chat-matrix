 ...
 * pace. `extension.ts` registers it inside the pi child, where it sits in the
 * same catalogue as Anthropic and OpenAI, so the agent loop, tool execution and
 * session persistence are all pi's own — only the model's choices are scripted.
 * Byte for byte the same script as `backends/pi/src/scripted.ts`.
 */
...
import type { Note, Weather } from "./tools.ts";
