 ...
 * written by the pi child as its run proceeds — there is no second store and
 * no "save on completion" step, because pi already owns a durable, append-only
 * record of every turn.
 *
 * The child opens its file by id (`--session-dir … --session-id <thread>`);
 * this module reads the same files back with pi's `SessionManager`, which is
 * the one place the server links pi as a library. Everything that runs a model
 * or a tool is in the child.
 *
